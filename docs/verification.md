...
|---|---|---|
| B0 | `python3 /opt/scripts/recon.py dns` | AXFR of `crosscreek-water.lab` and `crosscreek-power.lab` succeeds and lists every HMI/PLC/eng-ws by name; reverse sweep names .40.10/.11/.20/.21/.22. `dig axfr @172.30.10.53 crosscreek-water.lab` returns the zone. `nmap -Pn -sT plc-water.crosscreek-water.lab` (default top-1000 ports) shows only 8080 open; the Modbus port isn't in range, that's B1a. |
| B1 | `python3 /opt/scripts/recon.py sweep` then `recon.py creds` | sweep resolves the named hosts and reports none of the three field-protocol default ports (502/102/44818) open; creds prints `admin/admin -> ACCEPTED` for both HMIs |
| B1a | `python3 /opt/scripts/recon.py nmap` | full-range scan finds 10502 open on `plc-water`, 54818 on `plc-dosing`, 10102 on `plc-power` (plus 8080 on all three); the follow-on `-sV`/NSE stage correctly IDs the `8080` HTTP UIs as Werkzeug, and reports the field-protocol ports `unknown` (the NSE scripts are default-port-gated, that's expected, not a failure) |
| B1b | `python3 /opt/scripts/recon.py registers` | prints plc-water's coil/discrete/holding/input register values, plc-power's DB1 hex dump, and plc-dosing's dose/mode/logic tags |
...
| D2 | re-run B0, B1, B1a | B0: `dig axfr` returns `REFUSED`, `plc-water.crosscreek-water.lab` is `NXDOMAIN`, `nmap -sL 172.30.40.0/24` returns bare addresses. B1: `recon.py sweep` finds nothing; `creds` cannot reach the HMIs. B1a: `recon.py nmap`'s full-range scan from the edge times out on every OT host, no open ports (firewall drop, not a moved port) |
