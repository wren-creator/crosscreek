...
def api_state():
    ...
    return jsonify(
        ...
        sp=dict(
            gen_setpoint_mw=round(struct.unpack_from(">h", db, S.GEN_SETPOINT_MW_X10)[0] / 10.0, 1),
        ),
        diag=dict(
            plc_host=PLC_HOST,
            scan_count=struct.unpack_from(">h", db, S.SCAN_COUNT)[0],
        ),
    )
...
def api_cmd():
    ...
    if "sp" in cmd:
        c.db_write(S.DB_NUMBER, S.GEN_SETPOINT_MW_X10,
                   struct.pack(">h", int(round(float(cmd["sp"]) * 10))))
        return jsonify(ok=True)
