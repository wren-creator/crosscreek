...
Optional, self-paced, done after Session 7. Eight flags. Each is a value the
student can only produce by actually running the attack or the defense against
the range, not by reading the book.

## For the student

1. Bring the range up: `./start.sh` (flags 1–6) and later `./start.sh --segmented` (flags 7–8).
2. Work `tasks.md`.
