...
3. `modbus_attack.py stop-dist` — pressure collapses
4. blind the water HMI — hold IR mirror at nominal
5. `push_logic_water.py` — delete the high-level interlock
6. `cip_attack.py logic-push` — metering pump to 100%, interlock bypassed
7. `s7_attack.py trip feeder` + `trip load` — island + frequency excursion
8. `s7_attack.py stop` — freeze the RTU
- End state: tank overflowing, contaminated water to the main, HMI shows healthy, substation dark
